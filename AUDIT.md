...
|---|---|---|---|
| H1 | **Biais spatial des arbres** : quand le tirage dépasse le plafond de 6, `sorted(cols)[:MAX_TREES_PER_LINE]` conserve les plus petits x → arbres concentrés à gauche. Rare (espérance ≈ 3,4 arbres) mais faux. | `engine._generate_line` | tirer `rng.sample(range(L), k)` avec `k = min(tirage, plafond)` |
| H2 | **Divergence modèle/réalité sur la stagnation** : `next_state` ignore la mort par stagnation (120 ticks) et `MAX_TICKS`, que `step` applique. La recherche croit donc pouvoir « attendre indéfiniment » ; c'est la cause plausible des 6 défaites/20 (rivières à tronc unique, alignement hors horizon T+15). | `engine.next_state` vs `engine.step` | soit propager un compteur de stagnation dans l'état de recherche, soit pénaliser les chemins immobiles dans la priorité, soit horizon adaptatif (×2 quand le meilleur f stagne) |
| H3 | **Espacement « strictement 6 ou 12 » non tenu au raccord** : L = 19 n'étant multiple ni de 6 ni de 12, le wrap modulo crée un intervalle résiduel de 7 (ou 19−12) par ligne. | `Line.occupied` / `config.GRID_WIDTH` | passer L à 18 ou 24, ou documenter l'écart au raccord dans le README (actuellement muet) |
